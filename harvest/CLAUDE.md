...
Harvest 모듈은 타입 안전한 중앙집중식 카프카 설정 시스템을 사용합니다:

```python
from harvest.kafka import kafka_config, kafka_producer

# 설정 조회
servers = kafka_config.bootstrap_servers
topic = kafka_config.topic_name('data-collected')

# 메시지 전송  
kafka_producer.send('data-collected', {"data": "example"})
```

**주요 구성 요소**:

- `kafka-config/topics.yaml`: 전체 프로젝트의 토픽 정의
- `kafka-config/environments/`: 환경별 설정 오버라이드
- `src/harvest/kafka/`: Pydantic 기반 타입 안전한 로더

### 카프카 설정 테스트

```bash
# 타입 안전한 카프카 설정 종합 테스트
uv run python tests/test_type_safe_kafka.py
```
...
**중요**: 이 프로젝트는 Pydantic v2를 사용합니다. 다음 변경 사항에 주의하세요:
...
- `json_encoders` → removed (deprecated in v2)
...
config_dict = model.dict(by_alias=True)
...
config_dict = model.model_dump(by_alias=True)

model_config = ConfigDict(populate_by_name=True)
# json_encoders는 제거됨 - 다른 방식 사용
```
