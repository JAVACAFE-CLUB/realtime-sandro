...
from ..external.minio import MinIOClient, UploadProgress
from ..schemas.kafka import DataCollectedMessage, DataType
...
class UploadService:
    ...
    def find_bz2_files(self) -> List[Path]:
        ...
        Returns:
            .bz2 파일 경로 목록
        """
        return list(self.data_dir.glob("*.bz2"))
    ...
    def upload_file_with_retry(
        ...
        # 이미 업로드된 파일인지 확인
        if self.minio_client.object_exists(object_key, self.bucket_name):
        ...
        progress_callback = create_progress_callback(file_path.name)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if success:
                    # 업로드 검증
                    obj_info = self.minio_client.object_info(
                    ...
                    if obj_info:
                        ...
                        if local_size == remote_size:
                            ...
                            # 카프카 메시지 발송
                            self.send_data_collected_message(
                                file_path=file_path,
    ...
    def send_data_collected_message(
            self,
            file_path: Path,
            object_key: str,
            file_size: int,
        ...
        try:
            ...
            message = DataCollectedMessage(
                data_id=data_id,
                data_type=DataType.WIKI_DUMP,  # 현재는 위키 덤프만 처리
                ...
                metadata={
                    ...
                    "uploader": "upload_service",
                }
            )

            # 카프카 메시지 발송
            kafka_service = KafkaClient()
            success = kafka_service.send_data_collected(message)

            if success:
                logger.info(f"✅ 카프카 메시지 발송 완료: {data_id} -> {object_key}")
            else:
                logger.error(f"❌ 카프카 메시지 발송 실패: {data_id} -> {object_key}")

            return success

        except Exception as e:
            logger.error(f"카프카 메시지 발송 중 오류 발생: {e}")
            return False
