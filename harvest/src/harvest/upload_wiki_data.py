"""위키 데이터 파일을 MinIO에 업로드하는 메인 스크립트"""
...
from .minio_uploader import MinIOUploader, UploadProgress
...
class WikiDataUploader:
    ...
    def __init__(self, data_dir: Optional[str] = None, bucket_name: Optional[str] = None):
        """
        위키 데이터 업로더 초기화
        
        Args:
            data_dir: 위키 데이터가 있는 디렉토리 (None인 경우 설정에서 읽음)
    ...
    def create_progress_callback(self, filename: str):
        ...
        def progress_callback(progress: UploadProgress):
            ...
            if current_time - last_print_time[0] > 1.0 or progress.progress_percent >= 100:
                print(f"\r업로드 진행률 [{filename}]: {progress.progress_percent:.1f}% "
    ...
    def upload_file_with_retry(self, file_path: Path, max_retries: Optional[int] = None) -> bool:
        """
        재시도 로직을 포함한 파일 업로드
        ...
        # 이미 업로드된 파일인지 확인
        if self.uploader.object_exists(self.bucket_name, object_key):
        ...
        for attempt in range(max_retries):
            try:
                logger.info(f"업로드 시도 {attempt + 1}/{max_retries}: {file_path.name}")
                ...
                if success:
                    # 업로드 검증
                    obj_info = self.uploader.get_object_info(self.bucket_name, object_key)
                    if obj_info:
                        ...
                        if local_size == remote_size:
                            ...
                            logger.error(f"❌ 파일 크기 불일치: 로컬({local_size}) vs 원격({remote_size})")
                    else:
                        logger.error("❌ 업로드 검증 실패: 객체 정보를 가져올 수 없음")

                logger.warning(f"업로드 실패. 재시도합니다... ({attempt + 1}/{max_retries})")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)  # 환경변수에서 설정된 시간만큼 대기 후 재시도
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error(f"❌ 최대 재시도 횟수 초과. 업로드 실패: {file_path.name}")

        return False

    def upload_all_files(self) -> dict:
        """
        모든 .bz2 파일을 업로드
        ...
        print("\n" + "=" * 60)
        logger.info(f"업로드 완료 - 성공: {results['success']}, 실패: {results['failed']}")
...
def main():
    ...
    try:
        ...
        if results["failed"] > 0:
            logger.error(f"일부 파일 업로드에 실패했습니다. 실패: {results['failed']}개")
            sys.exit(1)
        else:
            logger.info("모든 파일 업로드가 성공적으로 완료되었습니다!")
